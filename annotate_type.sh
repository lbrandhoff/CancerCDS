...
intogen_annot_py=${dir}getIntogenAnnot_v2.py
...
echo "------------------------------------------------------------------"
echo "----Annotating ${vcf_in} with ${original_ctype}"
echo "------------------------------------------------------------------"

temp_dir=${out_dir}  # dir to store temp files in

mkdir -p ${temp_dir}  # makes tempdir
...
python3 $intogen_annot_py -i ${temp_dir}vep_prot.tsv -o ${temp_dir}intogen.tsv -c ${original_ctype}  # only non pan
